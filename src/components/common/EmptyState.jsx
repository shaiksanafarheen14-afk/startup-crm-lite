...
const EmptyState = ({ isTotalEmpty, onClearFilters }) => {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 bg-surface border border-border border-dashed rounded-xl text-center transition-colors duration-200">
      {isTotalEmpty ? (
        <>
          <div className="h-16 w-16 bg-surface rounded-full flex items-center justify-center mb-4 text-slate-400 dark:text-text-secondary transition-colors">
            <Inbox size={32} />
          </div>
        ...
        <>
          ...
          <h3 className="text-lg font-bold text-text mb-1">No leads found</h3>
          <p className="text-text-secondary max-w-sm mb-6">
